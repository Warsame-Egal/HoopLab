...
def merge_endpoint_config(**kwargs: Any) -> dict[str, Any]:
    merged = dict(kwargs)
    merged.update(get_api_kwargs())
    return merged


async def call_live_nba_api(
    factory: Callable[[dict[str, Any]], T],
    *,
    timeout: float = 10.0,
) -> T:
    """
    Live nba_api endpoints (scoreboard, boxscore, playbyplay).

    Matches nba-scoreboard: single rate_limit + asyncio.to_thread, no retries.
    Retries on empty JSON were causing live play-by-play to fail differently.
    """
    api_kwargs = get_api_kwargs()
    await rate_limit()
    return await asyncio.wait_for(
        asyncio.to_thread(lambda: factory(api_kwargs)),
        timeout=timeout,
    )
